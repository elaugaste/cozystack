...
func ProjectBackupCredentials(ctx context.Context, c client.Client, cfg BackupCredentialsConfig, targetNamespace string) error {
	...
	// Preflight ownership check. controllerutil.CreateOrUpdate would
	// happily overwrite anything sitting at the target name; refuse in
	// that case so a manually-created or restored Secret cannot be
	// silently clobbered by reconcile.
	target := &corev1.Secret{}
	getErr := c.Get(ctx, types.NamespacedName{Namespace: targetNamespace, Name: cfg.TargetSecretName}, target)
	if getErr == nil {
		...
		return &ProjectionError{
			Reason:  ReasonAPIError,
			Message: fmt.Sprintf("inspect target credentials Secret: %v", getErr),
		}
	} else {
		target = &corev1.Secret{
	...
	if _, err := controllerutil.CreateOrUpdate(ctx, c, target, func() error {
		...
		if creds.endpoint != "" {
			target.Data["endpoint"] = []byte(creds.endpoint)
		}
		if creds.bucket != "" {
			target.Data["bucketName"] = []byte(creds.bucket)
		}
		if creds.region != "" {
			target.Data["region"] = []byte(creds.region)
		}
		return nil
	}); err != nil {
		return &ProjectionError{
