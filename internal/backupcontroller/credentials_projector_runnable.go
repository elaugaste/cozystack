...
var _ manager.Runnable = (*SystemCredentialsProjector)(nil)
...
func (p *SystemCredentialsProjector) projectAll(ctx context.Context, logger logr.Logger) {
	for _, ns := range p.Namespaces {
		if err := ProjectBackupCredentials(ctx, p.Client, p.Config, ns); err != nil {
			...
			// Demote permanent-misconfig failures to Info-with-reason
			// after the first occurrence so a malformed source Secret
			// does not spam Error logs every minute forever; the
			// Prometheus counter keeps the signal visible.
